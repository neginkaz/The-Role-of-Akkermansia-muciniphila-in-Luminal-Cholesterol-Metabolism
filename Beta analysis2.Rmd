...
```{r pressure, echo=FALSE}
filename="otu_table_g_reversed.csv"
...
mds_data <- as.data.frame(mds$points)
write.table(mds_data,file="Brays.csv",row.names=FALSE,na="",col.names=TRUE,sep=",")

```

```{r pressure, echo=FALSE}
d= density(mds_data$MDS1)
df= read.csv("Bray.csv")
data= data.frame(df)

pdf("Bray_Density_NICE.pdf")
